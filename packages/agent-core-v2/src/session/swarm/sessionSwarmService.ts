...
export class SessionSwarmService implements ISessionSwarmService {
  ...
  private async spawnAttempt(
    ...
    try {
      ...
      throw wrapSubagentModelError(error, binding.model, callerData.modelAlias, options.binding?.source, options.profileName);
    }
    child.accessor
    ...
    emitAgentRunSpawned(caller, child.id, {
      ...
      model: subagentBindingDisplayModel(this.config, binding.model),
    });
      ...
      emitAgentRunSpawned(caller, agentId, {
        ...
        model:
          resumedModel === undefined
            ? undefined
            : subagentBindingDisplayModel(this.config, resumedModel),
      });
    }
