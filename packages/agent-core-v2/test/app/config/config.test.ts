...
describe('subagent config section', () => {
  ...
  it('resolves per-type derived ids to the agent_types base alias for display', async () => {
    const withType = await createConfig(
      {},
      '[agent_types.coder]\nmodel = "provider/coder"\nmax_output_size = 8192\n',
    );
    expect(subagentBindingDisplayModel(withType.config, agentTypeDerivedModelId('coder'))).toBe(
      'provider/coder',
    );
    expect(subagentBindingDisplayModel(withType.config, 'provider/coder')).toBe('provider/coder');
    withType.disposables.dispose();

    const bare = await createConfig({});
    expect(subagentBindingDisplayModel(bare.config, agentTypeDerivedModelId('coder'))).toBe(
      agentTypeDerivedModelId('coder'),
    );
  ...
  it('rejects force combined with a models table at spawn resolution, matching startup validation', async () => {
    ...
    expect(() => resolveSubagentBinding(config, secondaryModelFlags(), own)).toThrow(
      /\[secondary_model\]\.force cannot be combined with/,
    );
