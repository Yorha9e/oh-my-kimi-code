...
describe('AgentSwarmTool', () => {
  it('applies one subagent_type across templated subagents', async () => {
    ...
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), swarmMode, stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModelCatalog(), stubWorkspace(), stubLog());
    const input = {
  ...
  it('does not expose permission rule argument matching', () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModelCatalog(), stubWorkspace(), stubLog());
  ...
  it('passes the configured subagent timeout to swarm tasks', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ timeoutMs: 5_000 }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModelCatalog(), stubWorkspace(), stubLog());
  ...
  it('resolves spawn task bindings from the configured secondary model', async () => {
    ...
    expect(host.swarmService.run).toHaveBeenCalledWith(
      expect.objectContaining({
        tasks: [
          expect.objectContaining({ binding: { model: SECONDARY_DERIVED_MODEL_ID, thinking: 'low', source: 'secondary', displayModel: 'provider/secondary' } }),
          expect.objectContaining({ binding: { model: SECONDARY_DERIVED_MODEL_ID, thinking: 'low', source: 'secondary', displayModel: 'provider/secondary' } }),
        ],
      }),
    );
  ...
  it('resolves spawn task bindings from a per-type patch entry (derived id)', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ agentTypes: { coder: { model: 'provider/coder', maxOutputSize: 8192, thinking: 'medium' } } }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' }), stubModelCatalog(), stubWorkspace(), stubLog());
    ...
    expect(host.swarmService.run).toHaveBeenCalledWith(
      expect.objectContaining({
        tasks: [
          expect.objectContaining({ binding: { model: agentTypeDerivedModelId('coder'), thinking: 'medium', source: 'agent_types', displayModel: 'provider/coder' } }),
          expect.objectContaining({ binding: { model: agentTypeDerivedModelId('coder'), thinking: 'medium', source: 'agent_types', displayModel: 'provider/coder' } }),
        ],
      }),
    );
  ...
  it('lets the tool call opt back into the primary model', async () => {
    ...
    expect(host.swarmService.run).toHaveBeenCalledWith(
      expect.objectContaining({
        tasks: [
          expect.objectContaining({ binding: { model: 'main-model', thinking: 'high', source: 'own', displayModel: 'main-model' } }),
          expect.objectContaining({ binding: { model: 'main-model', thinking: 'high', source: 'own', displayModel: 'main-model' } }),
        ],
      }),
    );
  ...
  it('advertises both selectable models in the description only when configured', async () => {
    ...
    const unconfigured = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model' }), stubModelCatalog(), stubWorkspace(), stubLog());
  ...
  it('omits the capabilities suffix for models the catalog cannot resolve', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ model: 'provider/secondary' }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model' }), stubModelCatalog(), stubWorkspace(), stubLog());
