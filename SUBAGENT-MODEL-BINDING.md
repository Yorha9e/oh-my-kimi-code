...
- 两种绑定粒度：
  ...
  - **命名槽位（slot）**：实例级的隐式模型路由层。槽位是用户侧命名的模型路由，不向 LLM 暴露"选择模型/思考强度"的语义；LLM 面向子代理的唯一语义接口是 profile（`subagent_type`，对应用途/prompt/行为）。`binding_slot` 参数仅在用户、任务或 preset 显式点名槽位时原样透传槽位名，LLM 不得自造槽位名
...
例外：`Agent(resume=<id>, binding_slot=<name>)` 允许在唤回时用命名槽位**覆盖**子代理的模型，优先级为 slot 覆盖 > sticky 原配置——原模型被限流（429）或安全策略拦截时，换一个槽位继续会话而不丢上下文。覆盖会固化进子代理配置，此后的 resume / retry 沿用新模型；槽位未配置、未绑定模型或引用了未知别名时，保持原模型并在工具结果中给出警告。关闭本功能后 resume 的槽位覆盖同样被忽略。
