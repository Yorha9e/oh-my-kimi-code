...
`,
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ Oh My Kimi Code');
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/Kimi K2\s+Oh My Kimi Code ← current/);
    expect(pickerOutput).toMatch(/❯ Kimi Turbo\s+Oh My Kimi Code/);
