...
const executableName = process.platform === 'win32' ? 'omkc.exe' : 'omkc';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `omkc-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `omkc-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    const archivePath = resolve(artifactsDir, `omkc-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  omkc-${target}.zip\n`,
    );
  });
  ...
  it('produces a manifest from zip archive checksums', async () => {
    ...
    await writeFile(join(releaseDir, 'omkc-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'omkc-darwin-arm64.zip.sha256'),
      `${checksum}  omkc-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, 'oh-my-kimi-code@0.5.0']);
    ...
    expect(manifest).toEqual({
      ...
      platforms: {
        'darwin-arm64': {
          filename: 'omkc-darwin-arm64.zip',
          checksum,
        },
      },
    });
