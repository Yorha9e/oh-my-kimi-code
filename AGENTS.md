...
- `apps/kimi-code`: the CLI / TUI application. It consumes core capabilities through `@moonshot-ai/kimi-code-sdk` and must not depend directly on `@moonshot-ai/agent-core`. When writing or modifying its terminal UI, use the `write-tui` skill (`.agents/skills/write-tui/SKILL.md`). The omkc data home is `~/.omkc` (env priority `OMKC_HOME` > `KIMI_CODE_HOME` for compat, then the default); on first startup `src/migration/kimi-code-home.ts` copies user data over from the official `~/.kimi-code` (copy-only, marker `.migrated-from-kimi-code` suppresses re-runs). Project-scoped `.kimi-code/` dirs are intentionally unchanged.
...
- `pnpm-workspace.yaml` is the sole source of truth for workspace membership. It uses globs (`packages/*`, `apps/*`), so new packages land there automatically; there is no second list to keep in sync.
- The community edition does not ship a Nix build. `flake.nix` / `flake.lock`, the `nix-build.yml` workflow, and the `scripts/check-nix-workspace.mjs` workspace-sync check have been removed — do not reintroduce a `flake.nix` sync requirement.
...
- When packing the CLI for local install or release (`npm pack` in `apps/kimi-code`), always run `pnpm -C apps/kimi-code run build` first — the pack has no build hook and ships `dist/` as-is, so skipping the build installs a stale bundle whose `--version` still reads new. Verify the fresh bundle with a feature-unique symbol (`grep -c <symbol> apps/kimi-code/dist/main.mjs`) before packing and again in the installed copy after `npm install -g`; never verify with strings that already existed in the v1 engine (e.g. `binding_slot` — false positive), and never treat `--version` as proof of fresh code. Running sessions keep the old code in memory: new tool surfaces take effect only in newly started windows.
